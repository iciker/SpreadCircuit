...
**实测最低可用配置。** 我们实际以 4 vCPU / 8 GB RAM / 200 GB SSD 的机器长期运行过该节点，配合每 8 小时自动清理一次数据与日志，可以稳定完成同步并持续提供 EVM RPC，足以支撑本项目的报价与执行。官方规格面向的是通用非验证节点场景；仅作为本项目的 RPC 后端时，可以按此实测配置起步，再根据磁盘增长与内存水位调整。

节点默认可能产生约 100 GB/天的数据和日志，200 GB 磁盘在没有清理的情况下不到两天就会写满。清理策略是小规格机器能长期运行的前提，用 cron 每 8 小时执行一次：
...
```cron
# 每 8 小时清理超过 8 小时的节点数据与日志（按实际数据目录调整路径）
0 */8 * * * find ~/hl/data -type f -mmin +480 -delete 2>/dev/null; find ~/hl/data -type d -empty -delete 2>/dev/null
```

清理只影响历史数据文件，不影响节点继续同步与提供 RPC；`hl-visor` 会持续写入新块。首次上线后建议观察一两个清理周期，用 `df -h` 与 `du -sh ~/hl/data` 确认磁盘占用稳定在预期范围。

RPC 端口 3001 不应直接暴露到公网；远程访问时至少使用防火墙白名单、TLS、限流和访问控制。
